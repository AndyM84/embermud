...
int get_skill(CHAR_DATA *ch, int sn)
{
	...
	if (sn == -1) {            /* shorthand for level based skills */
		...
		if (sn == gsn_second_attack && (IS_SET(ch->act, ACT_WARRIOR) || IS_SET(ch->act, ACT_THIEF))) {
			...
			skill = 10 + 3 * ch->level;
		} else if (sn == gsn_disarm && (IS_SET(ch->off_flags, OFF_DISARM) || IS_SET(ch->off_flags, ACT_WARRIOR) || IS_SET(ch->off_flags, ACT_THIEF))) {
			skill = 20 + 3 * ch->level;
...
void affect_modify(CHAR_DATA *ch, AFFECT_DATA *paf, bool fAdd)
{
	 ...
	 */
	if (!IS_NPC(ch) && (wield = get_eq_char(ch, WEAR_WIELD)) != NULL && get_obj_weight(wield) > str_app[get_curr_stat(ch, STAT_STR)].wield) {
		static int depth;
...
void newaffect_join(CHAR_DATA *ch, NEWAFFECT_DATA *npaf)
{
	...
	for (npaf_old = ch->newaffected; npaf_old != NULL; npaf_old = npaf_old->next) {
