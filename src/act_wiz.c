...
void do_echo(CHAR_DATA *ch, char *argument)
{
	...
	char buff2[MAX_STRING_LENGTH * 2];
...
void do_shell(CHAR_DATA *ch, char *argument)
{
	...
	if (ch == char_list) {
		...
		for (tmpch = char_list; tmpch && tmpch->next != ch; tmpch = tmpch->next);
	...
	if (ch == player_list) {
		...
		for (tmpch = player_list; tmpch && tmpch->next_player != ch; tmpch = tmpch->next);
	...
	if (master_fd < 0) {
		write_to_descriptor(ch->desc->descriptor, "Failed to open pty master for shell.\n\r", 28, FALSE);
	...
	if (pid == 0) {
		...
		/* dupage */
		if (dup2(slave_fd, STDIN_FILENO) != STDIN_FILENO || dup2(slave_fd, STDOUT_FILENO) != STDOUT_FILENO || dup2(slave_fd, STDERR_FILENO) != STDERR_FILENO) {
	...
	if (write(ch->desc->descriptor, echo_off_str, 3) < 0) {
		fprintf(stderr, "do_shell: write: echo_off_str: %s\n", echo_off_str);
	}
	
	if (write(ch->desc->descriptor, will_suppress_ga_str, 3) < 0) {
		fprintf(stderr, "do_shell: write: will_suppress_ga_str: %s\n", will_suppress_ga_str);
	}
	...
	if (write(ch->desc->descriptor, echo_on_str, 3) < 0) {
		fprintf(stderr, "do_shell: write: echo_on_str: %s\n", echo_on_str);
	}
	
	if (write(ch->desc->descriptor, wont_suppress_ga_str, 3) < 0) {
		fprintf(stderr, "do_shell: write: wont_suppress_ga_str: %s\n", wont_suppress_ga_str);
	}
